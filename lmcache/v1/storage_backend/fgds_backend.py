...
class FgdsMemoryAllocator(GPUMemoryAllocator):
    def __init__(self, size: int, device=None):
        ...
        fgds_regmem(self.device_id, ctypes.c_void_p(self.base_pointer), ctypes.c_size_t(self.size), host_ptr)
        self.host_ptr = void_ptr

    def __del__(self):
        self.fgdsBufDeReg(self.device_id, ctypes.c_void_p(self.base_pointer), ctypes.c_size_t(self.size))
...
class FgdsBackend(AllocatorBackendInterface):
    """
    The FgdsBackend references GDSBackend. In order to use it, users need to specify
    `fgds_path` and `fgds_buffer_size` in their LMCache config.
    ...
    NOTE: The fgds should support on that filesystem.
    """
    ...
    def __init__(
        ...
        self._fgds_driver = self.fgds.FgdsDriver(self.device_id)
        ...
        if config.extra_config is not None:
            ...
            assert use_direct_io, "Fgds only support direct io mode"
        ...
        if hasattr(self.memory_allocator, "base_pointer"):
            ...
            logger.info("No base pointer found, fgds will use bounce buffers")
            ...
            for entry in it:
                ...
                tasks.append(
                    asyncio.to_thread(
                        ...
                        os.path.join(self.fgds_path, l1_dir),
                        l1_dir,
                    )
                )
    ...
    async def _async_save_bytes_to_disk(
        ...
        # only for the case where creating the fgds fails on ENOENT. It
        # also makes the code more resilient to out-of-band deletions
        if subdir_key not in self.metadata_dirs:
            os.makedirs(os.path.join(self.fgds_path, l1_dir, l2_dir), exist_ok=True)
            self.metadata_dirs.add(subdir_key)
    ...
    def _load_bytes_from_disk_with_memory(
        ...
        ret = self._load_fgds(path, offset, addr, memory_obj.get_size(), dev_offset)
    ...
    def _batched_get_blocking_by_thread_pool_impl(
        ...
        fgds_reads, fgds_read_bytes = 0, 0
        for dtype, shape, path in zip(dtypes, shapes, paths, strict=True):
            ...
            if memory_obj is None:
                ...
                fgds_reads += 1
                fgds_read_bytes += memory_obj.get_size()
        ...
        logger.info(
            ...
            f" {fgds_read_bytes / 1024 / 1024}MiB | {fgds_reads} ops."
        )
        return results
    ...
    def _load_fgds(
        ...
        if self.fgds:
            with self.fgds.Fgds(
                fgds_path, "r", use_direct_io=self.use_direct_io, device_id=self.device_id
            ) as f:
                return f.read(
                    gpu_pointer,
                    size_in_bytes,
            ...
            fd = os.open(fgds_path, os.O_RDONLY)
            ...
            raise RuntimeError(
                "Both fgds and cudart are None, this should not happen"
            )

    def pin(self, key: CacheEngineKey) -> bool:
        # NOTE (ApostaC): Since fgds doesn't have eviction now, we don't need
    ...
    def remove(self, key: CacheEngineKey, force: bool = True):
        raise NotImplementedError("FgdsBackend does not support remove now.")
    ...
    def allocate(
        ...
        if busy_loop:
            logger.warning("Fgds backend does not support allocation with busy loop")
        if eviction:
            logger.warning("Fgds backend does not support eviction")
    ...
    def close(self) -> None:
        ...
        logger.info("Fgds backend closed.")
